...
def render_paste_interface(
    ...
    if pasted_data and pasted_data.strip():
        if st.button(
            translations.get_text("process_data", lang_code),
            key=f"process_{key}",
            type="primary",
        ):
...
def parse_tsv_inventory(tsv_data: str) -> Dict[str, Dict[str, Any]]:
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if len(parts) < 2:
            logger.warning(
                f"Line {line_num}: Invalid format, expected building_id and quantity (and optionally era_level): {line}"
            )
            continue
        ...
        try:
            ...
            if quantity != int(quantity):
                logger.warning(
                    f"Line {line_num}: Decimal quantity {quantity} for {building_id}, converting to {int(quantity)}"
                )
            ...
            logger.warning(
                f"Line {line_num}: Invalid quantity '{parts[1]}' for building {building_id}"
            )
            continue
        ...
        if len(parts) >= 3:
            try:
                ...
                if era_level not in config.ERAS_LEVEL_MAP:
                    logger.warning(
                        f"Line {line_num}: Invalid era level {era_level} for {building_id}, ignoring era"
                    )
                    era_level = None
            except ValueError:
                logger.warning(
                    f"Line {line_num}: Invalid era level '{parts[2]}' for building {building_id}, ignoring era"
                )
                era_level = None
        ...
        if not building_id or len(building_id) < 3:
            logger.warning(
                f"Line {line_num}: Invalid building ID format: {building_id}"
            )
            continue
        ...
        if key in inventory:
            ...
            logger.warning(
                f"Line {line_num}: Duplicate building ID '{building_id}'. Adding {quantity} to existing {old_quantity} = {inventory[key]['quantity']}"
            )
        else:
            inventory[key] = {
    ...
    logger.info(
        f"Successfully parsed {len(inventory)} unique building entries from inventory"
    )
    return inventory
...
def parse_tsv_city(tsv_data: str) -> Dict[str, Dict[str, Any]]:
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if len(parts) != 3:
            logger.warning(
                f"Line {line_num}: City format requires 3 columns (building_id, era_level, quantity), got {len(parts)}: {line}"
            )
            continue
        ...
        try:
            ...
            if era_level not in config.ERAS_LEVEL_MAP:
                logger.warning(
                    f"Line {line_num}: Invalid era level {era_level} for {building_id}, skipping line"
                )
                continue
        except ValueError:
            logger.warning(
                f"Line {line_num}: Invalid era level '{parts[1]}' for building {building_id}, skipping line"
            )
            continue
        ...
        try:
            ...
            logger.warning(
                f"Line {line_num}: Invalid quantity '{parts[2]}' for building {building_id}, skipping line"
            )
            continue
        ...
        if unique_key in city_data:
            ...
            logger.warning(
                f"Line {line_num}: Duplicate building ID '{building_id}' at era {era_level}. Adding {quantity} to existing {old_quantity} = {city_data[unique_key]['quantity']}"
            )
        else:
    ...
    logger.info(
        f"TSV city parsing completed: {unique_buildings} unique buildings, {total_buildings} total buildings"
    )
...
def validate_building_data(
    building_data: Dict[str, Any], df_original: pd.DataFrame
    ...
    db_building_ids = (
        set(df_original["id"].unique()) if "id" in df_original.columns else set()
    )
    ...
    for unique_key, data in building_data.items():
        ...
        if building_id in db_building_ids:
            ...
            logger.info(
                f"Building ID '{building_id}' (from key '{unique_key}') not found in database"
            )
    ...
    try:
        ...
        log_entry = {
            ...
            "session_id": st.session_state.get(
                config.SessionKeys.SESSION_ID, "unknown"
            ),
        ...
        logger.info(
            f"Logged {len(unmatched_ids)} unmatched building IDs to {log_filepath}"
        )
...
def merge_with_database(
    ...
    """
    logger.info(
        f"Starting data merge for {source_type} with {len(building_data)} building types"
    )
    ...
    for unique_key, data in building_data.items():
        ...
        if building_rows.empty:
            logger.warning(
                f"Building ID '{building_id}' not found in database during merge (should have been filtered)"
            )
        ...
        if era_level is not None:
            ...
            if filtered.empty:
                logger.warning(
                    f"Building '{building_id}' not found in era level {era_level} ({config.ERAS_LEVEL_MAP.get(era_level)})"
                )
                continue
        ...
        for _, building_row in building_rows.iterrows():
            ...
            if source_type == "inventory":
                ...
                quantity = (
                    data.get("quantity", data.get("count", 1))
                    if isinstance(data, dict)
                    else 1
                )
                building_entry["Quantity"] = quantity
                ...
                if (
                    ...
                    building_entry["_coordinates"] = data[
                        "coordinates"
                    ]  # Private field
    ...
    logger.info(
        f"Created {len(building_entries)} building entries from {processed_buildings} database matches"
    )
    ...
    logger.info(
        f"Merge function: Weights active: {weights_active}, User weights: {user_weights}"
    )
    ...
    # Log summary statistics
    unique_buildings = (
        result_df[config.COL_NAME].nunique()
        if config.COL_NAME in result_df.columns
        else 0
    )
    logger.info(
        f"Merge completed: {len(result_df)} rows, {unique_buildings} unique buildings, {total_quantity} total items"
    )
...
def clear_all_data() -> int:
    ...
    try:
        ...
        logger.info(
            f"Cleared {cleared_count} city analysis data keys from session state"
        )
...
def render_city_analysis_tab(
    ...
    if config.SessionKeys.SESSION_ID not in st.session_state:
        ...
        st.session_state[config.SessionKeys.SESSION_ID] = str(uuid.uuid4())[
            :8
        ]  # Short session ID
    ...
    if st.session_state[config.SessionKeys.IMPORTED_INVENTORY] is None:
        st.session_state[config.SessionKeys.IMPORTED_INVENTORY] = (
            load_from_session_state("inventory_data")
        )
    if st.session_state[config.SessionKeys.IMPORTED_CITY] is None:
        st.session_state[config.SessionKeys.IMPORTED_CITY] = load_from_session_state(
            "city_data"
        )
    ...
    with col1:
        ...
        with st.expander(
            translations.get_text("how_to_import_inventory_data", lang_code)
        ):
            st.write(translations.get_text("inventory_analysis_help_import", lang_code))
            st.code(
                r"""copy(Object.values(MainParser.Inventory).filter((t=>"InventoryItem"===t.__class__&&"BuildingItemPayload"===t.item.__class__&&t.item.cityEntityId&&["A_","D_","G_","L_","M_","P_","R_","T_","W_","Z_"].some((e=>t.item.cityEntityId.startsWith(e))))).sort(((t,e)=>t.name.localeCompare(e.name))).map((t=>`${t.item.cityEntityId}\t${t.inStock}\t${t.item.level}`)).join("\n"));""",
                language="javascript",
                wrap_lines=False,
            )
        ...
        if inventory_data:
            try:
                ...
                valid_inventory, unmatched_ids = validate_building_data(
                    parsed_inventory, df_original
                )
                ...
                if valid_inventory:
                    ...
                    total_quantity = sum(
                        data.get("quantity", data) if isinstance(data, dict) else data
                        for data in valid_inventory.values()
                    )
                    ...
                    if unmatched_ids:
                        ...
                        log_unmatched_buildings(unmatched_ids)
                        warning_msg = (
                            f"{len(unmatched_ids)} building IDs not found in database"
                        )
                        show_toast_notification(warning_msg, "warning")
                else:
                    show_toast_notification(
                        "No valid buildings found in inventory data", "error"
                    )

            except ValueError as e:
                show_toast_notification(
                    f"{translations.get_text('data_corrupted', lang_code)}: {str(e)}",
                    "error",
                )
    ...
    with col2:
        ...
        if city_data:
            try:
                ...
                valid_city, unmatched_ids = validate_building_data(
                    parsed_city, df_original
                )
                ...
                if valid_city:
                    ...
                    # Show success notification
                    total_buildings = sum(
                        data["quantity"] for data in parsed_city.values()
                    )
                    ...
                    show_toast_notification(
                        "No valid buildings found in city data", "error"
                    )
    ...
    with col3:
        ...
        if st.button(
            translations.get_text("clear_all", lang_code),
            type="secondary",
            key="clear_all_data",
        ):
            # Clear session state
            ...
            # Show success notification
            show_toast_notification(
                translations.get_text("data_cleared", lang_code), "success"
            )
            st.rerun()
    ...
    if (
        st.session_state[config.SessionKeys.IMPORTED_INVENTORY]
        or st.session_state[config.SessionKeys.IMPORTED_CITY]
    ):
        st.markdown("---")
        ...
        if st.session_state[config.SessionKeys.IMPORTED_INVENTORY]:
            for unique_key, data in st.session_state[
                config.SessionKeys.IMPORTED_INVENTORY
            ].items():
                quantity = (
                    data.get("quantity", data) if isinstance(data, dict) else data
        ...
        if st.session_state[config.SessionKeys.IMPORTED_CITY]:
            for unique_key, data in st.session_state[
                ...
                if unique_key not in all_building_data:
                    ...
                    all_building_data[unique_key]["city_quantity"] = data.get(
                        "quantity", 1
                    )
        ...
        with st.spinner(translations.get_text("analysing_city", lang_code)):
            ...
            if st.session_state[config.SessionKeys.IMPORTED_CITY]:
                merged_city = merge_with_database(
                    st.session_state[config.SessionKeys.IMPORTED_CITY],
                    df_original,
                    source_type="city",
                    user_weights=user_weights,
                    user_context=user_context,
        ...
        if not df_imported.empty:
            ...
            if available_columns:
                ...
                eff_min = (
                    df_table[config.COL_WEIGHTED_EFFICIENCY].min()
                    if config.COL_WEIGHTED_EFFICIENCY in df_table.columns
                    and not df_table.empty
                    else 0
                )
                eff_max = (
                    df_table[config.COL_WEIGHTED_EFFICIENCY].max()
                    if config.COL_WEIGHTED_EFFICIENCY in df_table.columns
                    and not df_table.empty
                    else 0
                )
