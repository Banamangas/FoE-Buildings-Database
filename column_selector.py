...
class ColumnSelector:
    ...
    def _create_column_item(
        self, col: str, selected_columns: Set[str], key_suffix: str = ""
    ) -> bool:
        ...
        is_selected = col in st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET]
        # Include refresh counter in key to force widget recreation when buttons are clicked
        refresh_counter = st.session_state.get(
            config.SessionKeys.COLUMN_SELECTOR_REFRESH, 0
        )

        icon_b64 = (
            ui_components.get_icon_base64(col)
    ...
    def _apply_preset(self, preset_key: str, selected_columns: Set[str]) -> Set[str]:
        ...
        if preset_key in config.COLUMN_PRESETS:
            ...
            available_preset_columns = [
                ...
                if any(
                    col in group_cols for group_cols in self.available_columns.values()
                )
            ]
            return set(
                available_preset_columns + [config.COL_NAME]
            )  # Always include name
    ...
    def _sort_columns_by_group_order(self, selected_columns: Set[str]) -> List[str]:
        ...
        selected_list.sort(
            key=lambda col: column_order_map.get(col, 9999)
        )  # Unknown columns go to end
    ...
    def render_enhanced_column_selector(self, show_search: bool = True) -> List[str]:
        ...
        if config.SessionKeys.COLUMN_SELECTOR_REFRESH not in st.session_state:
            st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] = 0

        selected_columns = st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET]

        # Preset Selection
        with st.expander(
            "📋 " + translations.get_text("column_presets", lang_code=self.lang_code),
            expanded=True,
        ):
            ...
            with preset_cols[0]:
                if st.button(
                    translations.get_text(
                        "preset_basic_analysis", lang_code=self.lang_code
                    ),
                    width="stretch",
                    key="preset_basic",
                ):
                    st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = (
                        self._apply_preset("basic_analysis", selected_columns)
                    )
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()

                if st.button(
                    translations.get_text(
                        "preset_production_focus", lang_code=self.lang_code
                    ),
                    width="stretch",
                    key="preset_production",
                ):
                    st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = (
                        self._apply_preset("production_focus", selected_columns)
                    )
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()
                ...
                if st.button(
                    translations.get_text(
                        "preset_military_focus", lang_code=self.lang_code
                    ),
                    width="stretch",
                    key="preset_military",
                ):
                    st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = (
                        self._apply_preset("military_focus", selected_columns)
                    )
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()

                if st.button(
                    translations.get_text("preset_fsp_usage", lang_code=self.lang_code),
                    width="stretch",
                    key="preset_fsp",
                ):
                    st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = (
                        self._apply_preset("fsp_usage", selected_columns)
                    )
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()
            ...
            with preset_cols[1]:
                if st.button(
                    translations.get_text("preset_gbg_focus", lang_code=self.lang_code),
                    width="stretch",
                    key="preset_gbg",
                ):
                    st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = (
                        self._apply_preset("gbg_focus", selected_columns)
                    )
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()

                if st.button(
                    translations.get_text("preset_ge_focus", lang_code=self.lang_code),
                    width="stretch",
                    key="preset_ge",
                ):
                    st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = (
                        self._apply_preset("ge_focus", selected_columns)
                    )
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()

                if st.button(
                    translations.get_text("preset_qi_focus", lang_code=self.lang_code),
                    width="stretch",
                    key="preset_qi",
                ):
                    st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = (
                        self._apply_preset("qi_focus", selected_columns)
                    )
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()
                ...
                if st.button(
                    translations.get_text(
                        "preset_consumables_focus", lang_code=self.lang_code
                    ),
                    width="stretch",
                    key="preset_consumables",
                ):
                    st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = (
                        self._apply_preset("consumables_focus", selected_columns)
                    )
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()
        ...
        if show_search:
            st.subheader(
                "🔍 "
                + translations.get_text("search_columns", lang_code=self.lang_code)
            )
            search_term = st.text_input(
                translations.get_text(
                    "search_columns_placeholder", lang_code=self.lang_code
                ),
                key="column_search",
        ...
        if show_search:
            ...
            with action_cols[0]:
                if st.button(
                    translations.get_text(
                        "select_all_visible", lang_code=self.lang_code
                    ),
                    width="stretch",
                    ...
                    for group_cols in filtered_columns.values():
                        st.session_state[
                            config.SessionKeys.SELECTED_COLUMNS_SET
                        ].update(group_cols)
                    st.session_state[config.SessionKeys.COLUMN_SELECTOR_REFRESH] += 1
                    st.rerun()
            ...
            with action_cols[1]:
                if st.button(
                    translations.get_text(
                        "deselect_all_visible", lang_code=self.lang_code
                    ),
                    width="stretch",
                    key="deselect_all_visible",
                ):
                    for group_cols in filtered_columns.values():
                        st.session_state[
                            config.SessionKeys.SELECTED_COLUMNS_SET
                        ].difference_update(group_cols)
        ...
        if st.button(
            translations.get_text("clear_all_selections", lang_code=self.lang_code),
            width="stretch",
            key="clear_all",
        ):
            st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET] = {
        ...
        st.subheader(
            "📂 " + translations.get_text("columns_by_group", lang_code=self.lang_code)
        )

        if not filtered_columns:
            st.warning(
                translations.get_text(
                    "no_columns_match_search", lang_code=self.lang_code
                )
            )
        ...
        for group_key, group_columns in filtered_columns.items():
            ...
            with st.expander(
                f"{group_name} ({len(group_columns)} {translations.get_text('columns', self.lang_code)})",
                ...
                with group_cols[0]:
                    if st.button(
                        f"✅ {translations.get_text('select_all', self.lang_code)}",
                        key=f"select_all_{group_key}",
                        width="stretch",
                    ):
                        st.session_state[
                            config.SessionKeys.SELECTED_COLUMNS_SET
                        ...
                        st.session_state[
                            config.SessionKeys.COLUMN_SELECTOR_REFRESH
                        ] += 1
                        st.rerun()

                with group_cols[1]:
                    if st.button(
                        f"❌ {translations.get_text('deselect_all', self.lang_code)}",
                        key=f"deselect_all_{group_key}",
                        width="stretch",
                    ):
                        st.session_state[
                            config.SessionKeys.SELECTED_COLUMNS_SET
                        ].difference_update(group_columns)
                        # Keep name column
                        st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET].add(
                            config.COL_NAME
                        )
                ...
                for col in group_columns:
                    ...
                    new_selection = self._create_column_item(
                        col, selected_columns, f"_{group_key}"
                    )
                    ...
                    if (
                        new_selection
                        and col
                        not in st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET]
                    ):
                        st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET].add(
                            col
                        )
                        ...
                        not new_selection
                        and col
                        in st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET]
                    ):
                        st.session_state[
                            config.SessionKeys.SELECTED_COLUMNS_SET
                        ].discard(col)
        ...
        st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET].add(config.COL_NAME)

        if changes_made:
            st.rerun()

        # Sort selected columns according to COLUMN_GROUPS order
        return self._sort_columns_by_group_order(
            st.session_state[config.SessionKeys.SELECTED_COLUMNS_SET]
        )


def render_enhanced_column_selector(
    df_original: pd.DataFrame, lang_code: str, show_search: bool = True
) -> List[str]:
