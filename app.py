...
def main():
        ...
        if st.session_state[config.SessionKeys.ACTIVE_ANALYSIS_SUBTAB] == 0:
            try:
                ...
                if show_per_square and config.COL_SIZE in df_viz_filtered.columns:
                    divisor_col = df_viz_filtered[config.COL_SIZE].reindex(df_display.index, fill_value=1).astype(float).replace(0, 1)
    ...
    if st.session_state[config.SessionKeys.ACTIVE_MAIN_TAB] == 3:
        ...
        if show_per_square and config.COL_SIZE in df_viz_display.columns and not df_viz_display.empty:
            divisor_col = df_viz_display[config.COL_SIZE].replace([0, pd.NA], 1).astype(float)
